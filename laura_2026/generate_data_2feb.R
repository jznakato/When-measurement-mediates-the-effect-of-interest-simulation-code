...
# 
# # "A" because highly dependent on the arm
# get_delta_A<- function(A, E1, E2, W1, W2, W3, UE1, UE2, UDelta, verbose=F) {
#   
#   pscore_delta <- A*plogis(1.5+0.5*W1-0.5*W2-1*W3) +
#     (1-A)*plogis(-0.5+0.5*W1+0.5*W2+1*W3)
#   
#   if(verbose) { print(summary(pscore_delta)); hist(pscore_delta) }
#   Delta <- as.numeric(UDelta < pscore_delta)
#   
...
# 
# 
# # "Complex" because even after stratifying on cluster in Stage 1,
...
#   # 
#   
#   pscore_delta <- W3*plogis(0.85+0.4*A-2*W1+0.4*W2*A)+
#     (1-W3)*plogis(0.1+0.4*A+2*W1-0.4*W2*A)
#   
#   if(verbose) { print(summary(pscore_delta)); hist(pscore_delta) }
#   Delta <- as.numeric(UDelta < pscore_delta) 
#   
